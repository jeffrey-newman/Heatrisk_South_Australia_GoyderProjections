...
def calcEHF_recalc2(file_path, path_out, replicate, t95, q85, path_out_xl):
    ...
    xl_col_header = ('year', 'month', 'day', 'wState', 'rain', 'maxT', 'minT', 'srad', 'pres', 'apet', 'dmt', '3day', '30day', 'EHI_sig', 'EHI_accl', 'EHF', 'EHF_respec', 'severity', 'low', 'medium', 'high')
...
#    f_t95.write(str(t95))
    ...
    workbook = xlsxwriter.Workbook(path_out_xl)
    worksheet = workbook.add_worksheet();
    col = 0
    row = 0
    for header in xl_col_header:
    ...
    while not it.finished:
        ...
        dat = str(rd[0]) + "\t" + str(rd[1]) + "\t" + str(rd[2]) + "\t" + str(rd[3]) + "\t" + str(rd[4]) + "\t" + str(
            rd[5]) + "\t" + str(rd[6]) + "\t" + str(rd[7]) + "\t" + str(rd[8]) + "\t" + str(rd[9]) + "\t" + str(
            cd[0]) + "\t" + str(cd[0]) + "\t" + str(cd[1]) + "\t" + str(cd[2]) + "\t" + str(cd[3]) + "\t" + str(
            cd[4]) + "\t" + str(cd[5]) + "\t" + str(cd[6]) + "\t" + str(cd[7]) + "\n"
        f.write(dat)
        it.iternext()  
